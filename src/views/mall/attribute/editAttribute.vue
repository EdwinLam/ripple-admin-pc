...
<script>
  ...
  export default {
    ...
    methods: {
      show({typeItems,selectedType,item}){
        this.id = item.id
        this.visible = true
        this.typeItems = typeItems
        this.formValidate.typeId=item.typeId
        this.formValidate.attributeName=item.attributeName
      },
      returnToIndex(){
        this.visible = false
      ...
      handleSubmit (name) {
        this.$refs[name].validate(async(valid) => {
          if (valid) {
            const res = await MallAttributeApi.update(Object.assign({},this.formValidate,{id:this.id}))
